...
import { getProcessedLectureList } from "lib/client/lecture";

const POST_DATABASE_ID = process.env.NOTION_POST_DATABASE;
const LECTURE_DATABASE_ID = process.env.NOTION_LECTURE_DATABASE;
...
export const getLecture = async () => {
  const res = await notion.databases.query({
    database_id: LECTURE_DATABASE_ID,
  });

  const lectureList = getProcessedLectureList(
    res.results as unknown as global_types.Lecture[]
  );

  return lectureList;
};

export const getPagesFromDatabase =
  async (): Promise<global_types.PageList> => {
    const res = (await notion.databases.query({
...
export const getPagesByLecture = async () => {
  const res = await notion.databases.query({
    database_id: POST_DATABASE_ID,
  });
};
